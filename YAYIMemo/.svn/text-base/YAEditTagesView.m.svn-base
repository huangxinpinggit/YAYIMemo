...
@implementation YAEditTagesView
...
-(void)setModel:(YATagsModel *)model
{
    _model = model;
    if (model.selected == false){
        if (model.isInput) {
           [self.dataAry removeObject:model];
            ...
            for (YATagsModel *mdel in self.dataAry) {
...
-(void)selectedAction:(UITapGestureRecognizer *)tap{
    ...
    YATagsModel *model = self.dataAry[lab.tag - 100];
...
/*
 ...
 if (_dataAry.count >0) {
 if (i < self.dataAry.count) {
 NSString *tagName = self.dataAry[i];
 UILabel *label = [UILabel new];
 label.font = self.font;
 label.textColor = self.fontColor;
 label.textAlignment = NSTextAlignmentCenter;
 ...
 self.editText.frame = CGRectMake(self.leftMargin, (CGRectGetHeight(self.frame) - 26)/2.0, 60, 26);
 }
 }
