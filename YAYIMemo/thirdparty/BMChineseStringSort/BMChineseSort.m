...
+(NSMutableArray*)IndexWithArray:(NSArray*)objectArray Key:(NSString *)key{
    ...
    NSMutableArray *A_Result = [NSMutableArray array];
    //[A_Result addObject:UITableViewIndexSearch];
    NSString *tempString ;
...
+(NSMutableArray*)ReturnSortObjectArrar:(NSArray*)stringArr key:(NSString *)key{
    ...
    if ([chineseStringsArray count] >0) {
        BMChineseSort *chineseString = [chineseStringsArray firstObject];
        
        if ([chineseString.string isEqualToString:@"#"]) {
            [chineseStringsArray removeObject:chineseString];
            [chineseStringsArray addObject:chineseString];
        }
